<?php
		...
		*/
    	...
    	if(isset($_POST['next_current']) and ($_POST['next_current'] != NULL)){
			...
			$sql = "SELECT id as count FROM data_antrian_baru WHERE  STATUS = 3 ". $sql_loket .$filter_waktu. " ORDER BY id LIMIT 1";
    		...
    		if($rowClient['count']){
				...
				*/			
				$sqla = "SELECT id as count FROM data_antrian_baru WHERE  (status = 0 OR status = 1) ". $sql_loket .$filter_waktu." ORDER BY id LIMIT 1";
				...
				if($rows['count']){
					...
					if($model_antrian == 1){
						...
						$rstCountId = $mysqli->query("SELECT MAX(nomor) as count FROM data_antrian_baru WHERE STATUS = 2 ". $sql_loket.$filter_waktu." ORDER BY id ");
					}else{
						//Jika nomor antrian Tidak per loket
						$rstCountId = $mysqli->query("SELECT MIN(nomor) as count FROM data_antrian_baru WHERE STATUS = 3 ". $sql_loket.$filter_waktu." ORDER BY id ");
					}
					...
					if($model_antrian == 1){
						//Jika nomor antrian per loket
						$results = $mysqli->query('UPDATE data_antrian_baru SET status=0,nomor = '.$jmlCountId.' WHERE id='.$id.'');
					}else{
						//Jika nomor antrian Tidak per loket
						$results = $mysqli->query('UPDATE data_antrian_baru SET status=0 WHERE id='.$id.'');
					}
				...
				//insert
				$results = $mysqli->query('INSERT INTO data_antrian_baru (waktu,status) VALUES ("'.date("Y-m-d H:i:s").'",3)');
			}
			...
			$rstClient = $mysqli->query('SELECT count(*) as count FROM data_antrian_baru WHERE status!=3 ' . $sql_loket.$filter_waktu);
			$rowClient = $rstClient->fetch_array();
